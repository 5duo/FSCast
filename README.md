...
- **WiFi音频流** 🚧: 将音频通过WiFi传输到手机播放，解决车机蓝牙限制

### 🔊 WiFi音频流传输（开发中）
由于车机系统限制第三方应用无法使用蓝牙A2DP输出，FSCast通过WiFi将PCM音频实时传输到手机配套App：
- 车机端拦截ExoPlayer解码后的PCM音频，通过TCP发送给手机
- 手机端App接收PCM数据并通过AudioTrack/AudioRenderer播放
- 手机可输出到蓝牙耳机，绕过车机限制
- 支持多设备同时连接，车机端可选择切换
- 手机端可远程控制播放（播放/暂停/拖动/停止）
- 详见 [音频流协议文档](./docs/protocol/AUDIO_STREAMING_PROTOCOL.md)
...
- **[音频流传输协议](./docs/protocol/AUDIO_STREAMING_PROTOCOL.md)** - WiFi PCM音频流通信协议（三端共享）
...
| 应用 | 目录 | 平台 | 说明 |
|------|------|------|------|
| **FSCast** | `app/` | Android (车机) | DLNA投屏接收器 + 视频播放 |
| **FSCast Remote (鸿蒙)** | `companion-harmony/` | HarmonyOS | 手机端音频接收 + 播放控制 |
| **FSCast Remote (Android)** | `companion-android/` | Android | 手机端音频接收 + 播放控制 |
...
│       └── bluetooth/                # 蓝牙状态监听
│
├── companion-android/                # 手机端 Android App (FSCast Remote)
│   └── src/main/java/.../
│       ├── service/                  # TCP连接 + PCM播放
│       ├── ui/                       # Compose UI
│       └── bluetooth/                # 蓝牙状态监听
