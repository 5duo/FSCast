...
```
[0x02] [4B 数据长度 N] [8B 时间戳] [N 字节 PCM 数据]
```

| 字段 | 大小 | 说明 |
|------|------|------|
| 数据长度 | 4 bytes (int32 BE) | PCM 数据字节数 |
| 时间戳 | 8 bytes (int64 BE) | ExoPlayer 的 presentationTimeUs（微秒） |
| PCM 数据 | N bytes | 原始 PCM 16-bit 交错数据 |

**时间戳说明**：
- 时间戳来自 ExoPlayer 的 `presentationTimeUs`，表示此 PCM 数据应该在何时播放
- 单位是微秒（μs），基准是媒体开始时间
- 手机端可以使用时间戳来同步播放，避免长时间播放累积偏差
- 时间戳 = 0 表示无效时间戳（用于首次启动或 seek 后）
